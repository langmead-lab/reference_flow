...
int main(int argc, char** argv) {
    ...
    if (!strcmp(argv[optind], "split")){
        std::cerr <<
            "[split] Split a SAM file into high- and low-quality sub SAM files and " <<
        ...
        std::cerr <<
            "[merge] Merge a list of SAM/BAM files that contain the same set of reads " <<
            "in the same order. Best-ranked alignments will be selected in separate SAM/BAM " <<
